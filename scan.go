...
func GetScan(iface string) {
    subnet := getSubnet(iface)
    if subnet == nil {
        fmt.Println("subnet not found")
        return
    }
...
// Полное сканирование одного IP
func scanIP(ip string) Device {
    dev := Device{
        IP:       ip,
        MAC:      "unknown",
        Alive:    false,
...
func parseService(port int, banner string) ServiceInfo {
    s := ServiceInfo{Protocol: "tcp", Product: "unknown", Version: "unknown", Banner: banner}
...
func DetectInterfacePCAP(ifaceName string) (*pcap.Interface, *net.IPNet) {
    ifaces, _ := pcap.FindAllDevs()
    ...
    for _, iface := range ifaces {
        ...
        for _, addr := range iface.Addresses {
            ...
            // skip /32 (VPN, туннели, cellular, PPP)
            if ones == 32 {
                continue
            }

            // if bestNet empty — берём первую
...
// шире маска → выше ранг (как в оригинале)
